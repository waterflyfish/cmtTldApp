/*
 * EconCameraPipeline.cpp
 *
 *  Created on: Apr 20, 2017
 *      Author: patrick
 */

#include "EconCameraPipeline.h"
#include <gst/app/gstappsink.h>

namespace va {

EconCameraPipeline::EconCameraPipeline(string pipeName, string loggerName, string streamIp, int streamPort) :
...
EconCameraPipeline::~EconCameraPipeline() {
	// TODO Auto-generated destructor stub
}

Pipeline::PipelineState EconCameraPipeline::start() {
#ifdef DEBUG_BUILD
...
//			g_object_set(G_OBJECT(trackingConv), "flip-method", 2, NULL);
			...
			// do not emit signals: all calls will be synchronous and blocking
					    //gst_app_sink_set_emit_signals (appSink, 0);
			gst_bin_add_many(GST_BIN(pipeline), trackingTeeQueueTracking, trackingTeeQueueStream, source, trackingTee, encoder, payloader,
					sink, trackingConv, trackingSink, NULL);
...
//				return false;
//			}
			if (gst_element_link_many(trackingTee, trackingTeeQueueStream, encoder, NULL) != (gboolean) TRUE) {
#ifdef DEBUG_BUILD
				LOG4CXX_DEBUG(logger,
			...
			if (!running) {
				...
				LOG4CXX_INFO(logger, "[" << id <<"]START " << "SUCESS");
...
Pipeline::PipelineState EconCameraPipeline::init() {
#ifdef DEBUG_BUILD
	LOG4CXX_DEBUG(logger, "[" << id <<"] called");
#endif
	...
	if ((sourceCaps = gst_caps_from_string("video/x-raw, width=(int)1280, height=(int)720,framerate=60/1")) == NULL) {
#ifdef DEBUG_BUILD
...
//		return DONE;	}
	...
	if ((trackingConv = gst_element_factory_make("videoconvert", "trackingConv")) == NULL) {
#ifdef DEBUG_BUILD
		LOG4CXX_DEBUG(logger, "[" << id <<"]CSIDynamicPipeline::init() failed. Error with gst_element_factory_make('trackingConv')");
#endif
		return DONE;
	}
	if ((trackingConvCaps = gst_caps_from_string("video/x-raw,format=(string)GRAY8")) == NULL) {
#ifdef DEBUG_BUILD
...
//		return DONE;	}
	...
	if ((encoderCaps = gst_caps_from_string("video/x-h264, stream-format=(string)byte-stream, bitrate=(int)50000 ")) == NULL) {
#ifdef DEBUG_BUILD
		LOG4CXX_DEBUG(logger, "[" << id <<"]CSIDynamicPipeline::init() failed. Error with gst_caps_from_string()");
#endif
		return DONE;
...
Pipeline::PipelineState EconCameraPipeline::stop() {
	return NEW;
}

bool EconCameraPipeline::processMessage(Message* msg) {

}
GstElement* EconCameraPipeline::getSink(string sinkName) {
	return trackingSink;
}
GstElement* EconCameraPipeline::getdetectSink(string sinkName) {
	return pipeline;
}
GstElement* EconCameraPipeline::getdetectTee(string sinkName) {
	return trackingSink;
}
Pipeline::PipelineState EconCameraPipeline::detectinit() {
	...
	ss<<"video/x-raw, width=(int)1280, height=(int)720, framerate=30/1"<<" ! ";
	//ss<<"nvvidconv"<<" ! ";
	ss<<"tee name=t"<<" ! ";
	ss<<"queue"<<" ! ";
	ss<<"video/x-raw,format=(string)I420"<<" ! ";
	ss<<"omxh264enc"<<" ! ";
...
//	ss<<"t."<<" ! ";
	...
	printf("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++=");
	//if (!err) {Pipeline::PipelineState EconCameraPipeline::start() {
